...
namespace QualitestPageTest
{
    ...
    public class QualitestPageTest
    {
        ...
        {
            usersList = JsonConvert.DeserializeObject<List<User>>(File.ReadAllText("DataFile.json"));// I'm trying to read the Json file, then to start testing   
        }

        [Test, Category("Empty Fields")]  
        public void SendRequestWithEmptyField_SentFaild_ReturnsTrue()//UI test that validates if there's a field tha's empty
        {
       ...
       public void NavigateToContactUsPage(IWebDriver driver) 
        {
            driver.Navigate().GoToUrl("https://qualitestgroup.com/contact-us/");
